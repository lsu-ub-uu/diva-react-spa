...
import coraSearch from '../../__mocks__/coraSearch.json';
import coraSearchWithTwoRecordTypeToSearchIn from '../../__mocks__/coraSearchWithTwoRecordTypeToSearchIn.json';
...
describe('transformSearch', () => {
  ...
  it('Returns one BFFSearch entry with two recordTypeToSearchIn', () => {
    const searchDataList = transformCoraSearch(coraSearchWithTwoRecordTypeToSearchIn);
    expect(searchDataList[0]).toStrictEqual({
      id: 'personSearch',
      metadataId: 'personAutocompleteSearchGroup',
      presentationId: 'personAutocompleteSearchPGroup',
